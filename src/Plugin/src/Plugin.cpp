...
public:
  ...
  {
    return TH1D::Fill(x); //,*weight); // commented out weight as we don't have it yet for DARKLIGHT
  }
  Int_t Fill(Double_t x, Double_t w)
  {
    return TH1D::Fill(x, w); //*(*weight)); // commented out weight as we don't have it yet for DARKLIGHT
  }
};
...
public:
  ...
  {
    return TH2D::Fill(x, y); //,*weight); // commented out weight as we don't have it yet for DARKLIGHT
  }
  Int_t Fill(Double_t x, Double_t y, Double_t w)
  {
    return TH2D::Fill(x, y, w); //*(*weight)); // commented out weight as we don't have it yet for DARKLIGHT
  }
};
...
public:
  ...
  {
    return TH3D::Fill(x, y, z); //,*weight); // commented out weight as we don't have it yet for DARKLIGHT
  }
  Int_t Fill(Double_t x, Double_t y, Double_t z, Double_t w)
  {
    return TH3D::Fill(x, y, z, w); //*(*weight)); // commented out weight as we don't have it yet for DARKLIGHT
  }
};
...
TH1D *Plugin::dH1(const char *name, const char *title, int count, double from, double to)
{
  ...
  if (!hist)
  {
    ...
    if (getMemoryObject("Callbacks"))
      ((FrameworkCallbacks *)getMemoryObject("Callbacks"))->addHisto((std::string(path) + "/" + std::string(name)).c_str(), (TH1D *)hist);
    gErrorIgnoreLevel = 0;
    curr->cd();
  }
  return hist;
...
{
#ifdef WITHDB
  debug(99,"Saving %s %s %s\n",tagname, tablename, valstring);
  TObject *dummy = getMemoryObject("Runnumber");
  int runnumber = htonl(*(unsigned int *)dummy); //ugly but works
  ...
  while (conn==NULL)
    {
      conncount=0;
      debug(10,"Connecting\n");
      conn = PQconnectdb("port=8080 user=runinfo password=6DAmdasfdsvfsoe1210adask1 dbname=runinfo host=khronos.physics.sunysb.edu connect_timeout=5");
  ...
  int binary[3] = {0, 1, 0};
  debug(10, "Connection to database - OK\n");
...
#else
  debug(0, "DB link not enabled, trying to save to table %s field %s value %s\n", tablename, tagname, valstring);
#endif
}
